 ...
 * Drag state machine is in _drag-handler.ts (attached via attachDragHandler).
 *
 * Uses the Pointer Events API (PointerEvent, setPointerCapture) for unified
...
export abstract class BaseResizer {
  ...
  // Drag state (managed by _drag-handler.ts)
  ...
  // --- Public accessors for _drag-handler.ts ---
